...
function buildLengthRule(length) {
  if (length === "短诗") {
    return "正文写成一段左右, 控制在 2 到 8 句以内, 宁短勿长。";
  }
  if (length === "中诗" || length === "中等") {
    return "正文写成中等长度,  约 5 到 12 句, 保持克制和完整。";
  }
  if (length === "长诗") {
    return "正文可以适度展开为长诗, 约 8 句以上, 但不要冗长拖沓。";
  }
...
function buildPrompt(req, refs) {
  ...
  const titleRule = title
    ? `标题使用「${title}」, 第一行只输出这个标题, 单独成行, 随后空一行再写正文。诗的内容要扣题。`
    : `请自拟一个贴切的标题。第一行输出标题, 单独成行, 随后空一行再写正文。`;
