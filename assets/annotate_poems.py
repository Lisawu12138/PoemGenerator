#!/usr/bin/env python3
"""给 gucheng_poems.json 逐首添加 themes 与 style_notes 标注。

特性：
- 兼容任意 OpenAI 格式接口（默认 DeepSeek，可切 OpenAI 等）
- 断点续跑：已标注的诗会跳过，中断后重跑接着来
- 增量保存：每首标注完立即写盘，绝不丢数据
- JSON 容错 + 自动重试：模型偶尔抽风也能兜住
- 主题词表约束：保证 themes 标签一致、可统计

用法（PowerShell）：
    $env:LLM_API_KEY="sk-你的key"
    python annotate_poems.py
...
# ---------- 配置 ----------
INPUT_PATH = r"c:\Users\tianmacheng\Desktop\test\PoemGenerator\assets\gucheng_poems.json"
OUTPUT_PATH = r"c:\Users\tianmacheng\Desktop\test\PoemGenerator\assets\gucheng_poems_annotated.json"

API_KEY = os.environ.get("LLM_API_KEY", "")
BASE_URL = os.environ.get("LLM_BASE_URL", "https://api.deepseek.com/v1")
MODEL = os.environ.get("LLM_MODEL", "deepseek-chat")

SAVE_EVERY = 10         # 每标注 N 首写一次盘
MAX_RETRY = 3           # 单首最大重试次数
WORKERS = 12            # 并发线程数
...
SYSTEM_PROMPT = "你是一位资深的中国现代诗歌研究专家，尤其精通顾城的作品。"

USER_PROMPT_TMPL = """请分析下面这首顾城的诗，输出严格的 JSON。
...
def annotate_one(client, poem):
    ...
    for attempt in range(1, MAX_RETRY + 1):
        try:
            ...
            if not themes:
                raise ValueError("themes 为空或不在词表内")
